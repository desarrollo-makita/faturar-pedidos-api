const { obtenerPedidos, obtenerDataFacturamiento, facturarPedidos ,  updateFacturacionPedido} = require('../services/pedidosService.js');
...
async function facturacionPedidos(req, res) {
    try {
        ...
        if (!pedidos || pedidos.length === 0) {
            return  res.status(200).json({ mensaje: 'No existe data para procesar facturas' });
        }

        for(item of pedidos ){
            const extraerDataFacturamiento = await obtenerDataFacturamiento(item);
            
            logger.info(`Response extraerDataFacturamiento   ${JSON.stringify(extraerDataFacturamiento )}` );
            for(dataFactura of extraerDataFacturamiento){

                if(!dataFactura.Folio && !dataFactura.URL){
                    logger.info(`Pedido ${ dataFactura.FolioExterno }  sin Folio generado ` );
                    ...
                    logger.info(`Response responseFacturamiento   ${JSON.stringify(responseFacturamiento )}` );
                    if(responseFacturamiento.faturamento){
                        logger.info(`Pedido ${JSON.stringify(dataFactura.FolioExterno)} facturado correctamente , facturamiento telecontrol ${JSON.stringify(responseFacturamiento.faturamento)}` );
                        pedidosFacturados.push(dataFactura.FolioExterno);
                        await updateFacturacionPedido(dataFactura.FolioExterno , dataFactura.ID_Item);
                    }
                    
                }else {
                    logger.info(`Pedido ${ dataFactura.FolioExterno } con estado Facturado true` );
                    pedidosNoFacturados.push(dataFactura.FolioExterno);
                }
       ...
       res.status(200).json({pedidosFacturados : pedidosFacturados, pedidosNoFacturados :  pedidosNoFacturados});
